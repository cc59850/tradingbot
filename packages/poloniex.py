...
def make_currency_pair_string(currency_pair):
    ...
    return str(currency_pair.reference).upper()+'_'+str(currency_pair.base).upper()
...
class Poloniex(EXCHANGE.Exchange):
    ...
    def __init__(self,account, base_url=None):
        ...
        if not base_url is None:
            ...
            self.base_url=EXCHANGE.Exchange.MARKET_BASEURL_MAPPING['Poloniex']
    ...
    def depth(self,currency_pair, limit=50, raw=False, type='step2'):
        ...
        params='command=returnOrderBook&currencyPair='+make_currency_pair_string(currency_pair)+'&depth='+str(limit)
