...
def make_currency_pair_string(currency_pair):
    ...
    # return  str(currency_pair.base).upper() + '-' + str(currency_pair.reference)
    return Coinbase.COINNAMEMAPPING.get(str(currency_pair.base),str(currency_pair.base))+'-' +Coinbase.COINNAMEMAPPING.get(str(currency_pair.reference),str(currency_pair.reference))
...
class Coinbase(EXCHANGE.Exchange):
    ...
    def __init__(self,account, base_url=None):
        ...
        if not base_url is None:
            ...
            self.base_url=EXCHANGE.Exchange.MARKET_BASEURL_MAPPING['Coinbase']
