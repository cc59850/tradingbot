...
BASE_URL = EXCHANGE.Exchange.MARKET_BASEURL_MAPPING['AEX'] +"/"
...
class Client():
    ...
    def submitOrder(self, type, mk_type, price, amount, coinname):
        mk_type="cnc" if mk_type=="cny" else mk_type
        timestamp, MD5 = self.getMD5()
    ...
    def cancelOrder(self,mk_type,order_id,coinname):
        mk_type = "cnc" if mk_type == "cny" else mk_type
        timestamp, MD5 = self.getMD5()
        params = {'key': self.access_key, 'time': timestamp, 'md5': MD5, 'mk_type': mk_type, 'order_id': order_id,'coinname':coinname}
        return self.request("cancelorder", params)

    def getOrderList(self,coinname = None, mk_type=None):
        mk_type = "cnc" if mk_type == "cny" else mk_type
        timestamp, MD5 = self.getMD5()
        params = {'key': self.access_key, 'time': timestamp, 'md5': MD5, 'coinname':coinname, 'mk_type':mk_type}
    ...
    def getMyTradeList(self,mk_type='cny',coinname='bts',page=1):
        mk_type = "cnc" if mk_type == "cny" else mk_type
        timestamp, MD5 = self.getMD5()
        params = {'key': self.access_key, 'time': timestamp, 'md5': MD5, 'mk_type': mk_type, 'coinname': coinname, 'page':page-1}
