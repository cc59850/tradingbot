...
def make_currency_pair_string(currency_pair):
    result=KrakenRest.COINNAMEMAPPING.get(currency_pair.base,currency_pair.base) +'/' + KrakenRest.COINNAMEMAPPING.get(currency_pair.reference,currency_pair.reference)
    return result

def make_currency_pair_string_for_restful(currency_pair):
    # XXBTZUSD
    result = KrakenRest.COINNAMEMAPPING.get(currency_pair.base, currency_pair.base) + KrakenRest.COINNAMEMAPPING.get(
...
class KrakenRest(EXCHANGE.Exchange):
    ...
    def __init__(self,account, base_url=None):
        ...
        if not base_url is None:
            ...
            self.base_url=EXCHANGE.Exchange.MARKET_BASEURL_MAPPING['KrakenRest']
    ...
    def depth(self,currency_pair, limit=20, raw=False, type='step2'):
        # v1: 'https://api.kraken.com/0/public/Depth'
        '''
